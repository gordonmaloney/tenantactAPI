...
// ⭐ NEW: postcode normaliser (UK-style-ish: trim, uppercase, strip spaces)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⭐ NEW: derive and normalise postcode (if present)
    let postcode_hash;
    if (contactDeetsRaw) {
      const rawPostcode =
        contactDeetsRaw.postcode ??
        contactDeetsRaw.postalCode ??
        contactDeetsRaw.post_code;

      const normalizedPostcode = normalizePostcode(rawPostcode);

      if (normalizedPostcode) {
        // canonicalise back into the object under `postcode`
        contactDeetsRaw.postcode = normalizedPostcode;
        // optional: remove alternative keys if you want it tidy
        delete contactDeetsRaw.postalCode;
        delete contactDeetsRaw.post_code;

        // ⭐ NEW: postcode hash for lookups / dedupe / analytics
        postcode_hash = hmacId(normalizedPostcode);
      }
    }

    // Accept arbitrary complaintDeets object (already anonymised, no encryption needed)
    const complaintDeets =
      body.complaintDeets && typeof body.complaintDeets === "object"
        ? { ...body.complaintDeets }
        : undefined;
    ...
    const doc = {
      ...
      postcode_hash, // ⭐ NEW: deterministic hash for postcode
      ts: new Date(),
